...
void Motor::run() {
  ...
  if (!recoverMotorFault) {
    ...
    if (someFault){
      //bber100 need to add a test to include or not the mow motor 
      stopImmediately(false);
      recoverMotorFault = true;
      nextRecoverMotorFaultTime = millis() + 2000;                  
    ...
    if (millis() > nextRecoverMotorFaultTime){
      if (recoverMotorFault){
        ...
        if (recoverMotorFaultCounter >= 5){ // too many successive motor faults
          //stopImmediately(); //not needed because motorerror to true change op
          CONSOLE.println("ERROR: motor recovery failed after 5 retry");
          recoverMotorFaultCounter = 0;
          motorError = true;
        }
      } else {
